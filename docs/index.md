# Interfaz Gráfica para Control de Maqueta de Temperatura

Bienvenido a la documentación del proyecto **Interfaz en C# para control de una maqueta de temperatura**.

Este software permite controlar una planta de temperatura mediante una interfaz gráfica desarrollada en **Windows Forms**, comunicándose con una placa Arduino a través de comunicación serie USB.

## Características principales

- Control manual de actuadores (ventilador y calefactor)
- Aplicación de señales de entrada tipo escalón y rampa
- Control automático mediante controlador PID
- Visualización de temperatura en tiempo real
- Exportación de datos experimentales a CSV

## Contenido de la documentación

```{toctree}
:maxdepth: 2

installation
usage
architecture
development
